"""The bounded accessibility-tree walk.

Ported from typesafe-computer-use (https://github.com/awlevin/typesafe-computer-use, commit 24eb292,
typesafe_computer_use/ax_walk.py), Copyright (c) 2026 Aaron Levin, MIT License: see
LICENSES/typesafe-computer-use.txt. Changes: AxNode is defined here instead of in models.py; the visible area may have a non-zero origin
(x0, y0), so a window on a second display, or a single window's bounds, can be the area; nameless nodes are never de-duplicated, since WebKit nests
same-frame nameless containers (window > group > group > scroll area > web area) and keying them
dropped the whole page.

Takes children/attrs/actions as plain callables, so the pruning, de-duplication, and label-recovery
rules are testable against plain objects.
"""
...
def off_display(frame: Frame | None, display_w_pt: float, display_h_pt: float, x0: float = 0, y0: float = 0) -> bool:
    ...
    if w <= 0 or h <= 0:
        return False
    return x >= x0 + display_w_pt or y >= y0 + display_h_pt or x + w <= x0 or y + h <= y0


def center_on_display(frame: Frame | None, display_w_pt: float, display_h_pt: float, x0: float = 0, y0: float = 0) -> bool:
    ...
    if w <= 0 or h <= 0:
        return True
    return x0 <= x + w / 2 < x0 + display_w_pt and y0 <= y + h / 2 < y0 + display_h_pt
...
def subtree_key(role: str, label: str, frame: Frame | None) -> tuple | None:
    ...
    if frame is None or frame[2] <= 0 or frame[3] <= 0 or not label:
        return None
...
def walk_actionable(
    ...
    while queue:
        ...
        hidden = hidden or off_display(frame, display_w_pt, display_h_pt, x0, y0)
        ...
        visible = not hidden and clickable(frame) and center_on_display(frame, display_w_pt, display_h_pt, x0, y0)
