...
- `screen.scroll`: the frontmost window's main scrollable view (a named app must be the one in front). Native views: the largest scroll area's settable vertical scroll bar moves by a fraction (little 0.08, normal 0.25, a lot 0.6, or to the end). Web areas, which expose no bars: `AXScrollToVisible` on the text or link nearest to about three quarters of a screen beyond the edge (or the last one, for top/bottom). No wheel events and no pointer movement. At the edge, failed with "already at the top/bottom", nothing sent. Done only when the bar value or the positions of the first visible lines changed. Category `scroll`, Automatic by default.
- `pointer.click`: a real click (left, right or double) where the pointer already is; nothing moves it. The target is the app and window under the pointer at resolve. At dispatch, a pointer that moved or a different window under it fails with nothing clicked, so a confirmation can't approve one place and click another. Done only when that app's AX fingerprint changed, and "done" means only that. Category `click`.
