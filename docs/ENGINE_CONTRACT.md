...
- `screen.scroll`: the frontmost window's main scrollable view (a named app must be the one in front). Right before acting, even on Automatic, the same process must be in front with the same window. Native views: the largest scroll area's vertical scroll bar. Its value must read as a finite number in 0–1 and be settable, otherwise nothing is written. At the edge, "already at the top/bottom" is returned before any write. It moves by a fraction (little 0.08, normal 0.25, a lot 0.6, or to the end). Web areas, which expose no bars: `AXScrollToVisible` (checked available) on the text or link nearest to about three quarters of a screen beyond the edge. No wheel events and no pointer movement. Done only on scroll evidence: the bar value changed, or the targeted web element moved and is now inside the view. Anything else, including read failures, is unverified. The last element of a capped tree isn't claimed to be the page's end. Category `scroll`, Automatic by default.
